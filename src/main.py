import argparse
import os
from pathlib import Path
from datetime import datetime

# import methods
try:
    import lab_m1_1
except ModuleNotFoundError:
    lab_m1_1 = None
try:
    import lab_m1_2
except ModuleNotFoundError:
    lab_m1_2 = None

# global vars
img_default_path = Path(__file__).parent.parent/'images'

operations = {}
if lab_m1_1 is not None:
    operations.update(lab_m1_1.operations)
if lab_m1_2 is not None:
    operations.update(lab_m1_2.operations)

# main handler
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='arguments for pdi_lab')

    parser.add_argument('-i', '--input', type=str,default=os.path.join(img_default_path, 'input', 'donkey.jpg'), help='input path+file')
    parser.add_argument('-o', '--output', type=str,default=os.path.join(img_default_path, 'output', datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + '.png'), help='output path+file')
    parser.add_argument('-p', '--operation', type=str,required=True, help='operation to perform', choices=list(operations.keys()))
    parser.add_argument('-v', '--value', type=str, default='', help='value for operation')
    args = parser.parse_args()

    operations[args.operation](args.input, args.output, args.value)